...
as a plain reference document — read it before calling any of the three CLIs.
...
sandbox sysctl for codex, key injection for opencode). Don't improvise flags — the failure modes
are specific and the fixes are one-liners.
...
- **Parallel opencode needs a private data dir per process** (see the SKILL's `ocp()` helper) —
  otherwise you'll hit the shared-SQLite race.
